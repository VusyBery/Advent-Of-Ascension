...
public class ServerGamePacketListenerImplMixin {
	...
	public boolean aoa3$addCoyoteTime(boolean onGround, @Local(argsOnly = true, index = 1) ServerboundMovePlayerPacket packet) {
		return onGround || (packet.getY(this.player.getY()) == this.player.getY() + this.player.getJumpPower() && this.player.getDeltaMovement().y() < -this.player.getAttributeValue(Attributes.GRAVITY));
	}
}
